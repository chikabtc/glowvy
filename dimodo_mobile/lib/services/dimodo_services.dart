...
class DimodoServices implements BaseServices {
  ...
  @override
  Future<List<Review>> getReviews(id, int offset, int limit) async {
    try {
      ...
      final reviews = body['Data']['reviews'];
      if (body["Success"] == true) {
        List<Review> list = [];
        if (reviews == null) {
          return list;
  ...
  Future<List<Product>> fetchProductsByCategory({categoryId}) async {
    try {
      List<Product> list = [];
      var url =
          "http://dimodo.app/api/products/categories=$categoryId?start=0&count=200";
      print(url);
      final http.Response response =
  ...
  Future<List<Product>> fetchProductsByTag({tag}) async {
    try {
      List<Product> list = [];
      var url = "http://dimodo.app/api/products/tag=$tag?start=0&count=200";
      print(url);
